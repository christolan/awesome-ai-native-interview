...
**一句话：Transformer 推理就四个阶段——Attention（信息路由）→ FFN（知识存储）→ 层层堆叠（抽象升级）→ 解码（从向量到词）。一旦你把这四个阶段吃透，KV Cache 为什么吃内存、长上下文为什么贵、temperature 到底在调什么、模型为什么会产生幻觉、推测解码怎么加速——这些工程决策突然就变得自然而然了。**

---

## 阶段一：Attention——让每个 token "看到"其他所有 token

一个 token 刚进入 Transformer 层的时候，它对周围的 token 一无所知。注意力机制就是来解决这个问题的。
...
KV Cache 必须**按层**维护，这才是长上下文贵的真正原因。以 96 层、128K 上下文窗口、128 维 KV 头为例：`96 × 128K × 128 × 2（K+V）× 2 字节（FP16）`——光缓存就要吃掉大约 6GB 显存。这就是为什么 GQA（分组查询注意力）、KV 量化和滑动窗口注意力这些东西不是"锦上添花的优化"，而是生产部署的硬门槛。
...
- **贪心解码（temperature=0）：** 永远选概率最高的那个。确定、可复现，但通常无聊——产出容易重复、可预测
...
- 你会明白推测解码（用一个小草稿模型先快速提议 token，然后用完整模型一次前向传播验证）为什么能加速生成而不改变输出分布
