...
向量数据库用近似最近邻（ANN）索引来搞定这件事——HNSW、IVF 这些数据结构让检索快到毫秒级。同时它还提供了生产必需的周边：增删改查、元数据过滤、水平扩展。市面上的选择很多：Pinecone、Weaviate、Qdrant、Milvus、Chroma，各有优劣。
...
3. **检索**：拿这个向量去向量数据库里搜 Top K 个最相关的文档片段
...
**分块策略是被严重低估的坑。** 切太小，句子碎片离开上下文就失去了语义意义——一个孤立的半句话，Embedding 根本抓不准它的意思。切太大，浪费宝贵的上下文窗口 token，信噪比也往下掉。块多大合适？取决于文档类型：法律合同和 FAQ 页面的处理方式完全不同。光这一个决策，就能决定检索质量是优秀还是灾难。
...
- **Vector Database** 用 ANN 索引（HNSW、IVF）在百万级 Embedding 上做毫秒级近似最近邻搜索
