...
首先要刻进脑子里的认知：**思考 token 和输出 token 是完全相同的机制生成的。** 都是同一个自回归过程——模型根据前面所有 token 预测下一个 token，把它塞回上下文，重复。前向传播是同一个，权重是同一套。不存在什么"推理副脑"、辅助模块、神经网络里的推理模式开关。
...
这不是新发现——和思维链（chain-of-thought）提示背后的原理一模一样。新的地方在于通过强化学习把这套行为固化到了模型内部，模型会*主动选择*推理，用不着人来提示。机制没变，自主性变了。

---

## 推理开关贴在聊天模板上，不在模型里

如果模型内部没有推理开关，那它在哪里？在 **聊天模板层**——你的 JSON API 请求到模型吃进去的原始 token 流之间的翻译层。

每个聊天 API 都有这一层，哪怕你看不见。你发 `{"role": "user", "content": "你好"}`，聊天模板把它翻译成像 `<|user|>你好<|assistant|>` 这样的 token 序列。模型永远看不见 JSON——它只看见 token。聊天模板就是那个翻译中介。
...
- 对基座模型生成的优化——KV 缓存优化、推测解码、更快的注意力内核——直接提升推理性能，因为全都是同一个机制。
...
- **思考 token 和输出 token 由完全相同的自回归机制生成**——没有独立模型、没有辅助模块、神经网络内部没有模式开关。
- **推理有效的根本原因：外化了中间步骤**——每个 token 为下一个 token 丰富上下文，以单次输出生成无法做到的方式降低不确定性。
- **推理开/关在聊天模板层，不在模型里**——是 prompt 工程原语，通过注入或省略前缀中的特殊 token 实现。
- **像 DeepSeek-R1 的 `<｜end▁of▁thinking｜>` 和 ` ` 这样的边界标记，是强化学习习得的特殊 token**——功能上和 EOS token 同类。模型学会的是*什么时候*发它们，不是它们*怎么*工作。
- **一切都是 token**：推理、答案、切换、边界——没有分支，没有模式标志，只是同一个自回归过程中用不同前缀产生的不同行为。
