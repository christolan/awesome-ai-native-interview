...
- Token 是 LLM 的最小输入/输出单位
- Embedding 是将 token 序列映射为固定维度的语义向量
- Vector Database 按语义相似度高效检索向量
- Token → Embedding → Vector DB 构成了 RAG 的核心链路：问题 Token 化 → Embedding 成向量 → 从 Vector DB 检索相关内容 → 拼入上下文给 LLM
...
Embedding 和 vector database 并没有研究过，请直接告诉我。
